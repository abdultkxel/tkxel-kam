...
const baseConfig: IntegrationConfig[] = [
  ...
  ...(['jira', 'salesforce', 'hubspot', 'ms_teams'] as IntegrationSource[]).map(source => ({
    source,
    name: source === 'ms_teams' ? 'MS Teams' : source[0].toUpperCase() + source.slice(1),
    status: 'disconnected' as const,
    tokenStatus: 'missing' as const,
    autoCreate: false,
    deduplicationWindowMinutes: 30,
    accountMappingRules: ['Account name match'],
    eventTypeFilters: ['Escalation', 'Activity'],
    syncDirections: { title: 'inbound' as const, status: 'inbound' as const },
  })),
]
  ...
  errors: [
    {
      ...
      message: 'OAuth token missing. Re-authenticate before syncing.',
      payload: { status: 401 },
      critical: false,
    },
  ],
