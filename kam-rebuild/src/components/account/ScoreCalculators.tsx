import { CalendarClock, Check, CheckCircle2, ClipboardCheck, Info, Loader2, Play, Save, XCircle } from 'lucide-react'
...
function buildSummary(
  ...
  const risk = toPercent(scores.risk, 3)
  const projectedOverall = Math.round((relationship + contract + resource + csat + risk) / 5)
  ...
  return {
    ...
    csat: scores.csat,
    risk: scores.risk,
    legacyOverall: Number(((scores.relationship + scores.contract + scores.resource) / 3).toFixed(2)),
    projectedOverall,
    platformHealth: {
      ...
      <div className="tk-card p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h3 id="score-calculators-heading" className="text-base font-semibold text-ink">Score calculators</h3>
            <p className="mt-1 max-w-2xl text-sm text-ink-secondary">
              Tune the legacy health model and service coverage inputs before saving a new account score.
            </p>
          </div>
          <div className="grid gap-2 sm:grid-cols-3 lg:min-w-[520px]">
            <ScoreSummary label="Legacy score" value={`${summary.legacyOverall.toFixed(2)} / 3`} />
        ...
        {calculators.map(calculator => {
          ...
          return (
            <article key={calculator.id} className="tk-card overflow-hidden">
              <div className="flex flex-col gap-3 border-b border-surface-border bg-surface-secondary/70 px-4 py-4 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="divide-y divide-surface-border">
                {calculator.criteria.map(criterion => {
                  ...
                  return (
                    <div key={criterion.id} className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(0,1fr)_minmax(210px,260px)]">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="truncate text-sm font-medium text-ink">{criterion.name}</p>
...
function CriterionActivities({
  ...
  if (!tasks.length) {
    return (
      <div className="rounded-md border border-dashed border-surface-border bg-surface-secondary px-3 py-2 text-xs text-ink-secondary">
  ...
  return (
    <div className="rounded-md bg-surface-secondary px-3 py-3">
