...
- Stakeholder tab UI shows role, influence, relationship, sentiment, status, optional LinkedIn URL, coverage, activity fields, and LinkedIn links in the hierarchy; political risk is not exposed in the tab, form, or detail drawer. The add/edit stakeholder form and role filter read active stakeholder roles from Admin Planning configuration, while preserving existing custom role slugs already present on stakeholder records. Stakeholders without a `Reports to` value are presented as top-level/no-reporting-line hierarchy entries rather than errors.
...
- Seed data includes default stakeholder roles, gap rules, service catalog entries, compatibility adjacency rules, service bundles, growth recommendation rules, opportunity types, stages, and transitions.
