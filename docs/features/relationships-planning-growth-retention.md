...
- Account Stage > Growth is labeled service coverage and growth recommendations, with service search/category/status filters, paginated coverage rows, partial coverage saves, and recommendation cards that show base fit, account fit percentage, and dynamic context chips. Numeric score-factor bonus chips are hidden because backend factor increments are rule weights, not independently measured account metrics.
