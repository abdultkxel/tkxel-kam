...
import type { ScoringCriterion, RagLevel } from "@/data/healthScoring";
...
export function ScoringCard({ title, icon, criteria, selections, onSelect, score, interpret, scale }: Props) {
  const [open, setOpen] = useState(true);
  const result = interpret(score);

  const handleChange = (criterionId: string, value: string) => {
    onSelect({ ...selections, [criterionId]: parseFloat(value) });
  };
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="pt-2 border-t border-border">
            <div className="flex items-center justify-between text-xs mb-1">
              ...
              <span className={`font-bold ${RAG_STYLES[result.color].text}`}>{score.toFixed(2)} / 3</span>
            </div>
            <div className="h-2 rounded-full bg-border overflow-hidden">
              <div className={`h-full rounded-full ${RAG_STYLES[result.color].dot}`} style={{ width: `${(score / 3) * 100}%` }} />
            </div>
          </div>
        </div>
      </CollapsibleContent>
