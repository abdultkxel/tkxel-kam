...
_TABLE_BACKFILL_DEFAULTS: dict[str, dict[str, Any]] = {
    ...
    "service_growth_bundle_items": {
        "created_at": lambda: datetime.now(timezone.utc),
    },
    "service_growth_rules": {
        "source_selector_type": "service",
        "source_selector_value": "service",
        "target_selector_type": "service",
        "target_selector_value": "service",
        "base_fit_score": 70,
        "priority": 0,
        "rationale_template": "Configured growth rule.",
        "is_active": True,
        "created_at": lambda: datetime.now(timezone.utc),
        "updated_at": lambda: datetime.now(timezone.utc),
    },
    ...
    "kyc_agent_runs": {
        "queued_at": lambda: datetime.now(timezone.utc),
        "retry_count": 0,
        "max_retries": 0,
...
def apply_additive_migrations() -> None:
    ...
    migrate_missing_columns(
        [
            ...
            ServiceGrowthBundle.__table__,
            ServiceGrowthBundleItem.__table__,
            ServiceGrowthRule.__table__,
