...
from app.models import Account, Engagement, EngagementHealthSnapshot, EngagementRenewal, User
...
class EngagementService:
    ...
    def _retention_risk(account: Account, engagement: Engagement) -> str:
        if account.risk_status == "critical" or engagement.delivery_health < 60:
            return "critical"
        notice_days = EngagementService._days_until(engagement.notice_deadline)
        if notice_days is not None:
            if notice_days <= 14:
                return "critical"
            if notice_days <= 60:
                return "warning"
        renewal_days = EngagementService._days_until(engagement.renewal_date or engagement.end_date)
