...
class AllowedEmailDomainsUpdateRequest(BaseModel):
    model_config = ConfigDict(
        json_schema_extra={
            "examples": [
                {"domains_input": "tkxel.com, tkxel.io, camp1.tkxel.com, camp1.tkxel.io, camp2.tkxel.com, camp2.tkxel.io"},
                {"domains": ["tkxel.com", "tkxel.io", "camp1.tkxel.com", "camp1.tkxel.io", "camp2.tkxel.com", "camp2.tkxel.io"]},
            ]
        }
    )
...
class EngagementImportDraftUpdateRequest(BaseModel):
    model_config = ConfigDict(populate_by_name=True)
    ...
    @classmethod
    def optional_long_text_is_valid(cls, value: str | None) -> str | None:
        return validate_optional_long_text(value, "Engagement draft text")
    ...
    @model_validator(mode="after")
    def dates_are_consistent(self) -> "EngagementImportDraftUpdateRequest":
        if self.start_date and self.end_date and self.end_date <= self.start_date:
            raise ValueError("Engagement end date must be after start date.")
        return self


class EngagementImportDraftRejectRequest(BaseModel):
...
class EngagementImportDraftRead(BaseModel):
    ...
    source_document_id: str | None = None
    source_document_ids: list[str] = Field(default_factory=list)
    status: Literal["ready_for_review", "approved", "rejected"]
    name: str
    description: str | None = None
    owner_id: str | None = None
    owner_name: str | None = None
    ...
    resource_dependency_notes: str | None = None
    risks: list[str]
    stakeholder_drafts: list[dict[str, Any]] = Field(default_factory=list)
    source_citation: str | None = None
    confidence: int
    missing_fields: list[str] = Field(default_factory=list)
    created_by_id: str | None = None
    ...
    rejected_by_name: str | None = None
    rejection_reason: str | None = None
    approved_engagement_id: str | None = None
    created_at: datetime
    updated_at: datetime
    decided_at: datetime | None = None
    source_documents: list[SourceDocumentRead] = Field(default_factory=list)
...
class EngagementImportDraftPageRead(BaseModel):
    items: list[EngagementImportDraftRead]
    total: int
    page: int
    page_size: int
    pages: int
