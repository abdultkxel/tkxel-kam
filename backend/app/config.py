...
class Settings:
    def __init__(self) -> None:
        ...
        self.seed_default_role_users = os.getenv("SEED_DEFAULT_ROLE_USERS", "true").lower() == "true"
        self.cors_origins = [
            origin.strip()
        ...
        self.kam_ai_provider = os.getenv("KAM_AI_PROVIDER", "openai").strip().lower()
        self.kam_ai_api_key = os.getenv("KAM_AI_API_KEY", self.ai_kyc_api_key or os.getenv("OPENAI_API_KEY", "")).strip()
        self.kam_ai_model = os.getenv("KAM_AI_MODEL", self.ai_kyc_model or "gpt-5-mini").strip()
        self.kam_ai_base_url = os.getenv("KAM_AI_BASE_URL", "").strip()
        self.kam_ai_timeout_seconds = int(os.getenv("KAM_AI_TIMEOUT_SECONDS", "90"))
        self.kam_ai_max_retries = int(os.getenv("KAM_AI_MAX_RETRIES", "1"))
        self.kam_ai_retry_backoff_seconds = float(os.getenv("KAM_AI_RETRY_BACKOFF_SECONDS", "1.5"))
        self.kam_ai_max_input_tokens = int(os.getenv("KAM_AI_MAX_INPUT_TOKENS", "24000"))
        self.kam_ai_max_output_tokens = int(os.getenv("KAM_AI_MAX_OUTPUT_TOKENS", "3000"))
        self.kam_ai_temperature = float(os.getenv("KAM_AI_TEMPERATURE", "0.2"))
        self.kam_ai_max_cost_per_run_usd = float(os.getenv("KAM_AI_MAX_COST_PER_RUN_USD", "0.25"))
        self.kam_ai_embedding_provider = os.getenv("KAM_AI_EMBEDDING_PROVIDER", self.ai_kyc_embedding_provider).strip().lower()
        self.kam_ai_embedding_model = os.getenv("KAM_AI_EMBEDDING_MODEL", self.ai_kyc_embedding_model).strip()
        self.kam_ai_embedding_dimensions = int(os.getenv("KAM_AI_EMBEDDING_DIMENSIONS", str(self.ai_kyc_embedding_dimensions)))
        self.kam_ai_retrieval_top_k = int(os.getenv("KAM_AI_RETRIEVAL_TOP_K", "12"))
