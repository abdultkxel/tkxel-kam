...
@router.get("/kam-head-portfolio", response_model=DashboardRead, summary="KAM Head Portfolio dashboard", description="Returns portfolio widgets for health distribution, high-risk accounts, stale KYC, escalations, renewal focus, workload, overdue actions, governance cadence, and SLA compliance.")
...
@router.get("/leadership", response_model=DashboardRead, summary="Leadership dashboard", description="Returns read-only executive widgets for strategic health, retention, growth, revenue risk, major escalations, executive summaries, and decision queue.")
def leadership(
    current_user: Annotated[User, Depends(get_current_user)],
    service: Annotated[DashboardsService, Depends(get_dashboards_service)],
    search: str | None = None,
