...
@router.patch("/admin/playbook-templates/{template_id}", response_model=PlaybookTemplateRead, summary="Update playbook template", description="Updates template configuration, versions material changes, and preserves historical execution snapshots.")
...
@router.post("/playbooks/{template_id}/execute", response_model=PlaybookExecutionRead, status_code=status.HTTP_201_CREATED, summary="Execute playbook", description="Executes a confirmed active playbook template and creates account-specific tasks without silently changing health metrics.")
