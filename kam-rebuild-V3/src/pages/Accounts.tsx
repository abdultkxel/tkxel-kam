          ...
          <div className="flex min-w-0 flex-wrap gap-2">
            {visibleSavedViews.map(filter => (
              <div key={filter.id} className="inline-flex min-h-[44px] items-center gap-1 rounded-md border border-surface-border bg-surface-secondary p-1">
                <button className="min-h-[36px] rounded-md px-3 text-sm font-semibold text-ink transition-colors hover:bg-white" onClick={() => applySavedView(filter.id)}>{filter.name}</button>
                {user.role === 'admin' ? (
                  <button className="min-h-[36px] rounded-md px-2 text-xs font-semibold text-brand-blue transition-colors hover:bg-blue-tint-20" onClick={() => toggleFilterShared(filter.id)}>
