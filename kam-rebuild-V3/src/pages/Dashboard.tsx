...
export function Dashboard() {
  ...
  return (
    <div>
      <header className="mb-6 flex flex-col gap-4 border-b border-surface-border pb-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h1 className="font-display text-4xl font-bold leading-tight text-ink">My Dashboard</h1>
  ...
  return (
    <section className="tk-card p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <Link to="/tasks" className="tk-button-secondary min-h-[40px] px-3 py-2 text-xs">
          Open tasks
        </Link>
      </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          {to ? (
            <Link
              to={to}
              className="mt-3 inline-flex rounded-md text-ink transition-colors hover:text-brand-blue focus-visible:ring-2 focus-visible:ring-brand-blue/30"
            ...
            {rows.map(row => (
              <tr key={row.account.id}>
                <td className="px-5 py-4">
                  <div className="flex items-center gap-2">
                    <Link to={`/accounts/${row.account.id}`} className="font-semibold text-ink hover:text-brand-blue">{row.account.name}</Link>
