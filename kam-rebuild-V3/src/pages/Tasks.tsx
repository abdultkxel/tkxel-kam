    ...
    return (
      <article className={cn('rounded-lg border bg-white p-3 shadow-card', signal.severity === 'critical' ? 'border-rag-red/20' : signal.severity === 'warning' ? 'border-brand-orange/20' : 'border-surface-border')}>
        ...
        <div className="mt-3 grid gap-2">
          <Link className="tk-button-secondary min-h-[40px] bg-white text-xs" to={sourceRoute}>
            <ExternalLink className="h-4 w-4" />
            View source
          </Link>
          {signal.status === 'new' ? (
            <button className="tk-button-secondary min-h-[40px] bg-white text-xs" onClick={() => onReviewSignal(signal)}>
              <Play className="h-4 w-4" />
              Mark reviewed
            </button>
          ) : null}
          {!closed ? (
            <button className="tk-button-primary min-h-[40px] text-xs" onClick={() => onResolveSignal(signal)}>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <button
          type="button"
          className="flex h-10 w-10 shrink-0 cursor-grab items-center justify-center rounded-md border border-surface-border bg-white text-ink-secondary transition-colors hover:bg-surface-tertiary active:cursor-grabbing"
      ...
      <div className="mt-3 grid gap-2">
        <Link className="tk-button-secondary min-h-[40px] bg-white text-xs" to={`/accounts/${task.accountId}?tab=${isKycAgentTask ? 'kyc' : 'health'}`}>
          <ExternalLink className="h-4 w-4" />
          {isKycAgentTask ? 'KYC review' : 'Health tab'}
        </Link>
        {task.status === 'todo' ? (
          <button className="tk-button-secondary min-h-[40px] bg-white text-xs" onClick={() => onStartTask(task)}>
            <Play className="h-4 w-4" />
            Mark in progress
          </button>
        ) : null}
        {task.status !== 'done' && task.status !== 'skipped' ? (
          <button className="tk-button-primary min-h-[40px] text-xs" onClick={() => onCompleteTask(task)}>
            <CheckCircle2 className="h-4 w-4" />
            Complete with evidence
          </button>
        ) : null}
        {task.status !== 'done' && task.status !== 'skipped' ? (
          <button className="tk-button-secondary min-h-[40px] bg-white text-xs text-brand-orange" onClick={() => onSkipTask(task)}>
