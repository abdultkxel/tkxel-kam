...
## Engagement/SOW Management

Engagement/SOW records are supported under Account 360 and are API-backed when a user is authenticated. The frontend uses the V3 Zustand store as a UI cache after API responses, with local mock behavior only as a non-authenticated development fallback.

Implemented Engagement/SOW capabilities include:

```text
Engagement list on Account Overview and Account Engagements tab
Create engagement from manual entry
Edit engagement profile, commercial terms, owner, service lines, dates, source links, and risks
Archive engagement with permission checks
Engagement 360 detail view with profile, renewal, risks, activities, escalations, attachments, timeline, and health panels
Engagement health recalculation
Account Health rollup from engagement contributions
Historical Account Health rollup snapshots
Frontend loading, empty, and error states for engagement API calls
Frontend field-level validation display for FastAPI/Pydantic errors
```

Engagement/SOW backend APIs:
...
```text
Account360 loads engagement cards from GET /api/accounts/{account_id}/engagements
EngagementsPanel loads, filters, searches, sorts, paginates, and archives API-backed records
EngagementFormDialog calls POST/PATCH for create and edit
EngagementDetail calls GET /api/engagements/{engagement_id}
EngagementHealthPanel calls POST /api/engagements/{engagement_id}/health/recalculate
AccountHealthRollupPanel calls GET /api/accounts/{account_id}/health/rollup
```

Key validation and permissions:

```text
Required fields: account, engagement name, owner, start date, delivery status, and service line
End date must be after start date
Renewal date and notice deadline must match SOW terms
Value must be non-negative and use USD
Create/update: authorized KAM, KAM Head, Admin, or configured role
View: authorized account/engagement users and Leadership Viewer
Delete/archive: KAM Head/Admin/Super Admin or configured delete permission
```
...
Backend Engagement/SOW CRUD, filters, pagination, validation, permissions, health recalculation, and rollup snapshots
Backend RBAC seed cleanup for orphaned role-permission rows
Frontend login flow
...
Frontend Engagement/SOW screens are wired to backend APIs when an auth token is present
```
