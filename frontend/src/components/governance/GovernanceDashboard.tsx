      ...
      <div className="mt-5 rounded-lg border border-surface-border p-4">
        ...
        <div className="mt-2 grid grid-cols-7 gap-1">
          {days.map(day => {
            ...
            return (
              <button key={day.toISOString()} className="min-h-[54px] rounded-md border border-surface-border bg-white p-2 text-left hover:bg-surface-tertiary" onClick={() => setSelectedDay(day)}>
                <span className="text-xs font-semibold text-ink">{format(day, 'd')}</span>
                <span className="mt-2 flex min-h-[20px] flex-wrap gap-1">
                  {dayItems.slice(0, 3).map(item => (
                    <span key={item.id} className={cn('h-2.5 w-2.5 rounded-full', itemDotClass(item))} />
                  ))}
                </span>
              </button>
            )
